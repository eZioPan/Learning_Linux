...
[NOTE]
====
* 在容器运行的时候，如果你有使用 `sudo lsns --tree` 查看过系统上的各种命名空间，那么你可以发现，普通 rootless 容器是直接继承一个 `catatonit -P` 创建的用户命名空间的，而非直接在容器上创建的。这意味着，同一个常规用户下运行的所有常规容器，都公用同一个用户命名空间。这一点在我们之后解释容器安全性的时候会再次提及。

* 准确来说，我们从容器所在用户命名空间的**外部**查看容器的 uid_map 和 gid_map 返回的不一定是正确的信息。这是因为，即便是同一个 uid_map/gid_map 文件，不同的用户查看到的数据也可能是不同的，详见 `man 7 user_namespaces` 的 __User and group ID mappings: uid_map and gid_map__ 段落 。最正确的方法应该还是在容器中运行 `cat /proc/self/uid_map` 来获取更准确的信息。
====
