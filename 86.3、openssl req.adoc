...
`req` 命令主要用于创建和处理 PKCS#10 格式额证书请求。它可以额外地创建自签名证书，比如创建用于根 CA 的自签名证书。
...
-nameopt option::
决定如何显示主体名和签发机构名。`option` 参数可以是单个选项，或者用逗号分隔的多个选项。也可以多次设置 **-nameopt** 来设置多个选项。参见文档 link:https://www.openssl.org/docs/man1.1.1/man1/x509.html[x509(1)]。
