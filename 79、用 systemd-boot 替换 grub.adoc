...
若你在 Fedora 41 上进行替换，请预先禁用 kernel-install 配置文件 **/usr/lib/kernel/install.d/92-tuned.install**，再执行替换操作。**92-tuned.install** 会为 systemd-boot 管理的 entries 的 conf 文件的 **initrd** 行添加 `$tuned_initrd`,还会为 options 行添加 `$tuned_param`，但 systemd-boot 并不支持在配置条目中使用 `$` 标记的变量，因此会导致引导失败（主要是找不到 initrd）。

[source, sh]
----
# 禁用 92-tuned.install
ln -s /dev/null /etc/kernel/install.d/92-tuned.install
----

如果你__**不幸与笔者一样**__，在使用了 systemd-boot 的情况下，更新到了 Fedora 41，那么你立刻就可以在启动时体验到窗口只剩一行 **Error preparing initrd: not found** 的恐惧。

此时，你能做的就是，使用临时 shell 进行急救（当然，若你有 rescue image 启动项或者 livecd，也是不错的选择）
...
. 将该该设备挂在到 **/sysroot/boot** 下
. 进入 **/sysroot/boot/loader/entries/** 找到其中文件名具有 fc41 的 .conf 文件
. 将该 .conf 文件中 initrd 行的行尾的 `$tuned_initrd` 删除，保存，重启。
. 从我们刚刚修改过的启动项启动
. 然后执行上面提到的禁用 **92-tuned.install** 并重新生成启动项的操作。
...
systemd 使用命令 `kernel-install` 来管理启动项，而且 kernel-install 具有 systemd-boot 的配置文件。
