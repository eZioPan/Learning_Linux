...
允许 dnsmasq 的配置由 DBus 方法调用修改。可以被修改的配置为上游 DNS 服务器（以及对应的域名）以及清空缓存。需要 dnsmasq 构建为支持 DBus。若给出了服务名，那么 dnsmasq 将提供该名称的服务，而非默认值 **uk.org.thekelleys.dnsmasq**。
...
（仅 IPv6）指定 DHCPv6 服务器会使用的 server persistent UID UID。该选项通常不被需要，因为 dnsmasq 会在 DUID 首次被需要的时候自动创建它。当给出后，该选项为 dnsmasq 提供创建 DUID-EN 类型的 DUID 所需要的数据。注意，一旦设置，DUID 就会被存储在租用数据库中，所以在 DUID-EN 和自动创建的 DUID 之间的变换需要重新初始化租用数据库。__enterprise-id__ 由 IANA 分配，__uid__ 则是一个十六进制八位元组字符串，它对于特定的设备唯一。
...
环境变量继承于 dnsmasq 的调用者，会追加部分或全部的变量 +
