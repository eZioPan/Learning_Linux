...
-ext extension::
以文本的形式打印证书扩展。扩展用逗号分隔的字符串指定，比如“subjectAltName,subjectKeyIdentifier”。参见文档 xref:{x509v3_config_man5}[x509v3_config(5)] 了解扩展名。
...
-extensions section::
添加证书扩展的来源段。若该选项不存在，则扩展要么存在于不具名（默认）段，或者默认段应该包含一个称为 “extensions” 的变量，其指向要使用的段。参见 xref:{x509v3_config_man5}[x509v3_config(5)] 手册了解扩展节的格式详情。
