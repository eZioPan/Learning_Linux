...
添加如下的配置，可以让 dnf 快速跳过过慢的远程服务器，新增文件 **/etc/dnf/libdnf5.conf.d/99-minrate.conf**

[source, toml]
/etc/dnf/libdnf5.conf.d/99-minrate.conf
----
[main]
minrate = 500K #设置最小传输速率
timeout = 5 #当传输速率连续 5 秒小于我们设置的最小值的时候，放弃该服务器
----

另：临时使用可以在命令行中添加 `--setopt minrate=500K --setopt timeout=5` 来实现。
