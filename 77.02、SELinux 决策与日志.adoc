...
[TIP]
====
**auditd** 是可以接受 **SIGUSR1** 执行日志轮转的。

[source, sh]
----
kill -s USR1 $(pidof auditd)

# 当然，auditctl 除了接受 USR1 以外，还可以使用更人性化的信号名：
...
[TIP]
====
如果想查看所有类型的 audit 日志，则需要将 `avc` 修改为 `all`
====
...
[TIP]
====
当然，除了使用 checkpoint，还有另一种方法，那就是结合 auditd 的日志轮转功能，与 ausearch 可以指定 audit 日志文件的功能，来达成类似于捕获特定范围内日志的效果：

[source, sh]
----
# 在触发 AVC 阻止前进行 audit 日志轮转
auditctl --signal rotate

# 触发 AVC 阻止

# 【可选】触发后再次进行 audit 日志轮转
auditctl --signal rotate

# 使用指定文件的方式查询 AVC 错误
ausearch --input /var/log/audit.log.1 --message avc --interpret
# 或者你没有做最后的日志轮转
ausearch --input /var/log/audit.log --message avc --interpret
----
...
[IMPORTANT]
====
遇到了 SELinux 问题，必然是要查询 SELinux 帮助的。有很多域是有自己的 manpage 的，这些 manpage 中包含了与该域相关的各种类型的说明。而这些 manpage 的名称均具有如下的形式 **<域名称>_selinux**，比如 **httpd_selinux** 就是介绍 **httpd** 这个域相关的 selinux 信息的。
